...
| File | What | Shapes |
|---|---|---|
| `style_encoder_qwen.onnx` | **the shipped head** — "fh5_d10": PCA-256 front + 512×2 MLP, trained on the qwen-relabeled 514-pair bank (24 MB) | `embedding [1, 2048]` (Qwen3-TTS speaker features) → `style_ttl [1, 50, 256]`, `style_dp [1, 8, 16]` |
...
| `style_basis.bin` | v5 PCA style basis (k=384, refit on the qwen-relabeled bank) for the refine search | little-endian: `int32` header (k, D, split, ttl_r, ttl_c, dp_r, dp_c) then `scale[k]`, `mean[D]`, `basis[k·D]` |
...
the shipped head scores **0.588** held-out over 77 voices — above the **0.4905**
